...
# Register (or update) a Cloud Scheduler entry that triggers the withings-refresh
# Cloud Run Job every 6 hours. Each run re-pulls a trailing window
# (app.withings.refresh-window-days, default 14d) of every connected user's
# Withings data, so the dashboard stays fresh even when notify callbacks are
# missed or the subscription lapses.
#
# Why every 6h: webhooks are the primary, low-latency path; this is only the
# safety net. Four runs a day keeps data current without hammering the API, and
# the 14-day window means a single missed run never leaves a gap. Mirrors
# bootstrap-gh-refresh-scheduler.sh.
#
# Auth: Cloud Scheduler hits the Cloud Run Admin API
# (`run.googleapis.com/.../jobs/{name}:run`) signed as the runtime service
# account via OAuth. The Job itself is not publicly reachable.
#
# Idempotent: `describe` decides between create and update.
#
# Prerequisite: the Cloud Run Job withings-refresh must already exist. Run
# `bash infra/scripts/deploy-withings-refresh-job.sh` first.

PROJECT_ID="${PROJECT_ID:-health-fitness-160}"
REGION="${REGION:-us-central1}"
JOB_NAME="withings-refresh"
SCHEDULER_NAME="withings-refresh-6h"
...
# PERMISSION_DENIED (code 7) and silently creates no execution, so grant it up
# front. Idempotent — re-adding an existing binding is a no-op.
