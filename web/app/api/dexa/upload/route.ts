...
// Proxies a DEXA upload to the backend so the browser can stream the
// SSE response without holding the user's Google ID token. The browser
// can't call the backend directly — the bearer header lives in the
// Auth.js session, which is server-side only.
//
...
export async function POST(request: Request) {
  ...
  const res = await apiFetch("/api/me/dexa/scans", {
    method: "POST",
    body: forwarded,
