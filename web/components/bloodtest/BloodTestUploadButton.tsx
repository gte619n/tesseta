...
// /api/blood/upload (which proxies to the backend with auth).
//
...
//
// Phase strings mirror the backend contract in BloodTestController:
//   uploading  → "Saving your PDF"
//   extracting → "Reading the report with Gemini"
...
export function BloodTestUploadButton() {
  ...
  async function uploadOne(state: FileState, file: File) {
    ...
    let res: Response;
    try {
      res = await fetch("/api/blood/upload", { method: "POST", body: fd });
    } catch (err) {
      updateOne(state.id, {
        status: "failed",
  ...
  return (
    <>
      ...
      <button
        ...
        disabled={busy}
        title="Drag blood test PDFs onto the page, or click to choose"
      ...
      >
        ...
        <span>Drop Lab Result PDFs</span>
      </button>
          ...
          <div className="w-[440px] rounded-[14px] border-[0.5px] border-border-default bg-surface px-7 py-6 shadow-[0_24px_64px_rgba(0,0,0,0.12)]">
            <div className="flex items-baseline justify-between">
              <div className="text-[15px] font-medium tracking-[-0.01em] text-primary">
                {allSettled
                  ? failedCount === 0
                    ? `Saved ${doneCount} report${doneCount === 1 ? "" : "s"}`
                    : `${doneCount}/${files.length} saved`
                  : `Reading ${files.length} report${files.length === 1 ? "" : "s"}`}
              </div>
              {!busy && (
