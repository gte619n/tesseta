...
> Evidence: `core/medication/ReminderSettingsService.java`, `TimeSlot.java`; nutrition parts in `Macros.java`/`NutritionService.java`.
> Successor / current source of truth: docs/reference/feature-catalog.md. **Part A (reminders) further superseded by IMPL-21** (archived alongside: ../plans/IMPL-21-single-rolling-medication-reminder.md).
> Note: this is the **med-reminders/nutrition-UX IMPL-16** — NOT the active-workout-logging branch effort that also went by "IMPL-16".
