...
> Evidence: documents three completed refactor passes; e.g. the extracted hook at `web/lib/use-chat-stream.ts`, consumed by `GoalsChat.tsx` and `WorkoutProgramChat.tsx`.
> Successor / current source of truth: none (completed refactor audit).
