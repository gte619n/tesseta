...
> Evidence: `ProgressionEngine.java`, `ProgressionMath.java`, `ProgressionController`, `BodyweightClassifier.java`.
> Successor / current source of truth: docs/reference/feature-catalog.md.
> **Correction:** the in-doc status block still says 🟡 "no code yet" — that is wrong; the engine shipped (see `ProgressionEngine.java`).
...
**Created:** 2026-09-05
**Source spec:** `tesseta-progression-engine-spec.md` (Draft v1; external draft — never checked into the repo) — this plan **adapts** that spec to the shipped codebase and records where we diverge.
