...
> Evidence: `BodyweightClassifier.java` (single source of truth), `SeedWeightResolver`, android inline RIR on `ActiveRepCard`, delta chips.
> Successor / current source of truth: docs/reference/feature-catalog.md.
> **Correction:** the in-doc status block still says 🟡 "no code yet" — that is wrong; shipped (see `ProgressionEngine.java`/`BodyweightClassifier.java`).
