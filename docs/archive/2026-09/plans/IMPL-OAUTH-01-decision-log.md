...
> Evidence: `api/platform/OAuthAuthorizationController.java`, `OAuthTokenController.java`, `api/v1/*Controller.java`, `platform/PlatformTokenService.java`.
> Successor / current source of truth: docs/decisions/ADR-0020-third-party-oauth-platform-api.md (the decision of record).
> Note: **renamed** from `docs/decisions/ADR-0020-implementation-decision-log.md` — this was never an ADR (a D1–D23 implementation log squatting on the ADR-0020 number); relocated per reaping-plan §4 to leave ADR-0020 unambiguous.
